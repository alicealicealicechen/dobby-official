...
客戶從官網送出的詢問會同時進到兩個地方：**寄一封通知信**，以及**存進這裡**。兩邊獨立運作，其中一邊出問題另一邊還在，訊息不會遺失。

這些內容是**唯讀的**，你只能改一個欄位：

**處理狀態** — 未處理 / 已回覆 / 已關閉

清單上前面有 **●** 的是還沒處理的。回覆完客戶記得改狀態，團隊才知道進度。

### 注意「通知信已寄出」欄位

如果某一筆顯示 **false**，代表通知信寄失敗了，**只有後台這一筆記錄**。這種情況要主動聯繫客戶，並通知網站管理者去查寄信服務。
...
後台的內容清單裡有一個叫 **頁面** 的類型。**現在不要用它。**

前台還沒有對應的路由，你就算建立並發佈了，訪客打開會看到 404 錯誤，但它又會被收進網站地圖交給 Google。等工程端補上支援之後才能使用。
